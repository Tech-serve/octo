...
const config = {
  ...
  // Сколько РАЗ повторить упавшую задачу при транзиентной ошибке (обрыв сессии,
  // таймаут перехода, не открылся reply-бокс и т.п.). Не ретраим «не подтверждён»
  // (мог запоститься — дубль) и бан/чекпоинт. По умолчанию 1 повтор = 2 попытки.
  taskRetries: int(process.env.TASK_RETRIES, 1),
  ...
  postDelayMaxMs: Math.max(0, num(process.env.POST_DELAY_MAX, 3)) * 60000,
  ...
  // Роли, которым разрешён вход в бота. Открыто баерам (+ head/admin).
  botAllowedRoles: (process.env.BOT_ALLOWED_ROLES || 'buyer,head,admin')
